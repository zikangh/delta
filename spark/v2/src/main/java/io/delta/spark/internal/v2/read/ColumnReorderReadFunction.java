...
/**
 * Reorders a Parquet reader's {@code readDataSchema ++ partitionSchema} output into the DDL order
 * advertised by {@link SparkScan#readSchema()}.
 */
public class ColumnReorderReadFunction
  ...
  private Iterator<InternalRow> applyRow(PartitionedFile file) {
    ProjectingInternalRow projection =
        ProjectingInternalRow.apply(targetSchema, scala.Predef.wrapIntArray(columnMapping).toSeq());
  ...
  private ColumnarBatch reorderBatch(ColumnarBatch batch) {
    ColumnVector[] reordered = new ColumnVector[columnMapping.length];
    for (int i = 0; i < columnMapping.length; i++) {
      reordered[i] = batch.column(columnMapping[i]);
    }
    return new ColumnarBatch(reordered, batch.numRows());
  }

  /**
   * Factory method. Returns the {@code baseReadFunc} unchanged if the columnMapping is a no-op (no
   * reorder needed - saves a per-batch allocation).
   */
  public static Function1<PartitionedFile, Iterator<InternalRow>> wrap(
    ...
    int[] columnMapping = computeColumnMapping(sourceSchema, targetSchema);
    if (isNoOpColumnMapping(columnMapping)) {
      return baseReadFunc;
    }
    return new ColumnReorderReadFunction(
        baseReadFunc, isVectorizedReader, targetSchema, columnMapping);
  }

  /**
   * Reconstructs the reader's output column layout: {@code dataSchema} fields, then {@code
   ...
   * sourceSchema}.
   */
  private static int[] computeColumnMapping(StructType sourceSchema, StructType targetSchema) {
    ...
    int[] columnMapping = new int[targetFields.length];
  ...
  private static boolean isNoOpColumnMapping(int[] columnMapping) {
    for (int i = 0; i < columnMapping.length; i++) {
      if (columnMapping[i] != i) {
        return false;
      }
    }
    return true;
